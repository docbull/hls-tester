    ...
    (typeof self !== 'undefined' ? self : this, function () {
        var IPFSHlsMultiChunk=(()=>{
            ...
            var w=g((p,b)=>{
                ...
                // chunkReader reads video chunk from local storage
                async function chunkReader(file) {
                    return new Promise(resolve => {
                        fetch(file)
                        .then(response => response.arrayBuffer())
                        .then(function(text) {
                            let units = new Uint8Array(text)
                            console.log("docbull", units);
                            resolve(units);
                        })
                    })
                }
                ...
                async function m(n,t,e,i,a){
                    ...
                    // If the requested chunk is stored in local storage,
                    // it loads the chunk.
                    // This is working fine with HLS.js, but it have to 
                    // run IPFS in background. 
                    let units = await chunkReader(`/${n}`);
                    console.log('docbull end')
                    o.push(units);
                    l+=units.length;
                    if(a[0]){
